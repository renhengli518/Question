...
(function() {
	...
	window.UEDITOR_CONFIG = {
		...
		// 黏贴是否要去掉样式，true表示去除，默认不去除
		// ,removePasteStyle:'true'
		// 服务器统一请求接口路径
		,
		...
		,
		toolbars : [ [ 'fullscreen', 'undo', 'redo', '|', 'cleardoc',
				'kityformula', 'bold', 'italic', 'underline', 'fontborder',
				'strikethrough', 'superscript', 'subscript', 'removeformat',
				'autotypeset', 'forecolor',
				'backcolor', 'insertorderedlist', 'insertunorderedlist',
				'selectall', 'simpleupload' ] ]
		...
		// 纯文本粘贴模式下的过滤规则
		/*
		 * 'filterTxtRules' : function(){ function transP(node){ node.tagName =
		 * 'p'; node.setStyle(); } return { //直接删除及其字节点内容 '-' : 'script style
		 * object iframe embed input select', 'p': {$:{}}, 'br':{$:{}},
		 * 'div':{'$':{}}, 'li':{'$':{}}, 'caption':transP, 'th':transP,
		 * 'tr':transP,
		 * 'h1':transP,'h2':transP,'h3':transP,'h4':transP,'h5':transP,'h6':transP,
		 * 'td':function(node){ //没有内容的td直接删掉 var txt = !!node.innerText();
		 * if(txt){ node.parentNode.insertAfter(UE.uNode.createText(' &nbsp;
