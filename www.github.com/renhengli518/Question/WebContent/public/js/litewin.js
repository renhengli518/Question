...
(function() {
	...
	function showDialog(cfg) {
		var config = { //默认值设置。
			...
			drag: true,
			bound: true,
			extend : false, //最大化，最小化
			locked: false
		};
		extendCopy(cfg || {}, config); //对象拷贝继承
		...
		domHTML += '<h3 class="dialog_Head"><b>' + config.title + '</b><span class="dialog_Opts"><i class="dialog_close">×</i>';
		domHTML += (config.extend) ? '<i class="dialog_restore">❐</i><i class="dialog_max">□</i><i class="dialog_min">▂</i>' : '';
		domHTML += '</span></h3>';
		...
		(function setting() {
			...
			if (config.locked) _locker(true);
			_pos();
			domIdx++;
			events.addEvent(closeBtn, 'click', _close);
			events.addEvent(dialogHead, 'mousedown', _drag);
			events.addEvent($class("dialog_max", dom)[0],'click',function(){
				_locker(true);
				//addClass(dom,"maxDialog");
			});
			events.addEvent($class("dialog_restore", dom)[0],'click',function(){
				_locker(false);
				//removeClass(dom,"maxDialog");
			});
		})();
		
		function _locker(flag){
			if(flag){
				lockedNum++;
		...
		function _close() {
			...
			if (dom) {
				...
				if (config.locked) _locker(false); //去除locked
			};
	...
	var Win = {
		...
		confirm: function(config, ok, cancel) {
			...
			events.addEvent(okBtn, 'click', function() {
				if (typeof(ok) == 'function' && ok() === false) return;
				myConfirm.close();
			});
