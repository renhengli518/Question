(function ($, undefined) {
	...
	JsTree.prototype = {
		...
		rushNode: function (id, opt) {
			...
			this.options.onExpand.call(self, this.nodeMap[id], $li, function () {			
		...
		delData: function (id) {
			...
			if (id == 'jsTree') {
				...
				if (this.options.delCheckLeaf && childs.length == 0) {
					...
					$parent.removeClass('jsTree-leaf').find('ul:eq(0),.jsTree-expandBtn:eq(0)').remove();
				}
			}
		},
