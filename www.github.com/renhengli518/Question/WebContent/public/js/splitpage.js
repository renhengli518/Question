...
SplitPage.prototype = {
	...
	ajaxGet: function(func, needToAppend) {
		...
		var _apply = function(msg) {
			...
			if (that.callback) that.callback(msg.data, msg.total, that.req, msg.map, needToAppend); //total:总页数；req:原请求参数；needToAppend:数据是否append到页面
